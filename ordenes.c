...
void guardarCSV(Orden ordenes[], int cantidad) {
    ...
    if (archivo == NULL) {
        ...
        printf("Datos guardados correctamente en ordenes.csv.\n");
    }
}
...
void cargarCSV(Orden ordenes[], int *cantidad) {
    ...
    if (archivo == NULL) {
        printf("No existe archivo ordenes.csv. Se iniciara sin datos.\n");
    } else {
        fgets(cabecera, 200, archivo);
 
        while (*cantidad < 100 &&
               fscanf(archivo, "%15[^,],%49[^,],%39[^,],%59[^,],%f,%d\n",
